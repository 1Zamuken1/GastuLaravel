    ...
    <div class="modal-content">
        <div class="modal-header" style="background: #ffebee;">
            <div class="modal-title">
                ...
                <span>Recordatorio de proyección, ¿quieres registrar este egreso proyectado?</span>
            </div>
            <button class="close-modal" id="closeRecordatorioModal" type="button">&times;</button>
        </div>
        <div class="modal-body">
            <form id="formRecordatorio" method="POST" action="/proyecciones_egresos">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" id="recordatorio_concepto_id" name="concepto_egreso_id" />
                </div>
                <div class="form-group">
...
<script>
    document.getElementById("formRecordatorio").addEventListener("submit", function(e) {
        ...
        .then(data => {
            if (data.status === 201) {
                // Redirige a la vista principal de egresos
                window.location.href = "{{ route('egresos.index') }}";
            } else if (data.errors) {
